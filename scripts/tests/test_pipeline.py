...
import assign_ecoregion_colours as colour  # noqa: E402
import extract_ecoregion_shapes as shapes  # noqa: E402
import reconcile_ecoregions as reconcile  # noqa: E402
...
# ----------------------------------------------------------------------------------------
# Ecoregion map: shapes, boundaries and colours
# ----------------------------------------------------------------------------------------


class TestEcoregionMap:
    """The map is generated by three scripts writing one file in sequence, and the output
    is committed rather than built. These pin the properties that made the generation worth
    doing, so a regeneration that quietly loses one fails here instead of on the page.
    """
    ...
    def _shapes() -> dict:
        ...
        path = Path(__file__).resolve().parent.parent.parent / "src/data/ecoregion-shapes.json"
        return json.loads(path.read_text())

    def test_every_region_has_a_ring_a_label_and_a_fill(self):
        data = self._shapes()
        slugs = set(shapes.BY_NUMBER.values())
        assert set(data["regions"]) == slugs
        assert set(data["label_points"]) == slugs
        assert set(data["region_fill"]) == slugs

    def test_the_three_letter_slot_key_is_gone(self):
        # Superseded by region_fill; a component reading it would silently paint every
        # region the same colour rather than fail.
        assert "colour_slot" not in self._shapes()

    def test_labels_sit_inside_their_own_region(self):
        # A bounds centre falls outside any concave shape — Oceanic's would print its "0"
        # on top of its neighbour's "1".
        data = self._shapes()
        for slug, (x, y) in data["label_points"].items():
            assert shapes.point_in_ring(x, y, data["regions"][slug]), slug

    def test_neighbouring_regions_are_distinguishable_under_colour_blindness(self):
        data = self._shapes()
        fill = data["region_fill"]
        pairs = colour.adjacency(data["regions"])
        assert len(pairs) >= 10, "adjacency detection found almost no shared borders"
        worst = min(colour.separation(fill[a], fill[b]) for a, b in pairs)
        # 8 is the dataviz target; 6 is the floor that is only legal with secondary
        # encoding, which the numbers and legend do provide.
        assert worst >= 8, f"weakest neighbouring pair is delta E {worst:.1f}"

    def test_regions_meet_without_large_gaps_or_overlaps(self):
        # The traced hit targets were drawn independently and did not tile; reconciling
        # them is what stops slivers of basemap and double-tinted seams.
        overlap, gaps = reconcile.measure(
            {k: [list(p) for p in v] for k, v in self._shapes()["regions"].items()}
        )
